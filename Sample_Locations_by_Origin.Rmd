...
join_all  %>% 
  ...
  labs(colour = "Origin") +  # change legend title
  guides(colour = guide_legend(override.aes = list(size = 4))) +  # make legend line width larger ~ pseudo fill
  stat_density_2d(data = filter(.data = join_all, origin == "Natural"), aes(fill = ..level..), geom = "polygon", alpha = 0.5, colour = origin_colors[1], show.legend = FALSE) +  # plot natural
  scale_fill_gradient(low = "white", high = origin_colors[1]) +  # add custom fill colour
  new_scale_fill() +  # switch fill
  stat_density_2d(data = filter(.data = join_all, origin == "Hatchery"), aes(fill = ..level..), geom = "polygon", alpha = 0.5, colour = origin_colors[2], show.legend = FALSE) +  # add hatchery
  scale_fill_gradient(low = "white", high = origin_colors[2]) +  # add custom fill colour
